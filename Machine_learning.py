...
def extract_data():
    ...
    for i in [9,10,12,15,18,19,24,26,27,30]:
        ...
        for num_segment in range(len(os.listdir(repertoire_geste))):
            ...
            #data_distance=data_distance_brut[0]+data_distance_brut[1]+data_distance_brut[2]+data_distance_brut[3]+data_distance_brut[4]+data_distance_brut[5]+data_distance_brut[6]
        ...
        if len(data_g)!=len(data_m):
            ...
            for k in range(len(data_g)):
                #print("-------------------------------------------------------------------")
                #print(data_g[k],data_s[k], data_au[k],data_e[k],data_m[k])
...
def learn(X,Y,Y_tag,cross_validation=False,draw_graph_each = False):
    ...
    for i in range(0,10): # chois l'interraction qui est en Test
        ...
        Y_train_tag=[]
        Y_test_tag=Y_tag[i]
        for j in range(0,10): #l'ajoute toutes les données sauf celle de l'interraction chosis pour le test
            if j!=i:
                X_train+= X[j]
                Y_train+= Y[j]
                Y_train_tag+=Y_tag[j]
        ...
        #print(X_train[25])
        if cross_validation and False:
            grid_search = GridSearchCV(model, param_grid, cv=5, n_jobs=-1)  # Utilisation de n_jobs=-1 pour paralléliser les calculs
    ...
    if True:
        ...
        if False:
            model=RandomForestClassifier(max_depth=150, min_samples_leaf=32,n_estimators=500)
            model.fit(X_train,Y_train_tag)
            predictions = model.predict(X_test)
            #print(predictions)
            accuracy = accuracy_score(Y_test_tag, predictions)
            score = model.score(X_test, Y_test_tag)
            ...
            if False:
                ...
                model.add(Dense(128, activation='relu', input_shape=(69,)))  # Couche d'entrée
